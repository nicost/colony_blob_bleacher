...
data_path = "/Users/xiaoweiyan/Dropbox/LAB/ValeLab/Projects/SG/Data/20210301_Jose_SGanalysis40xObjective/"

thresholding = 'local-sg1'  # only accepts 'na', 'otsu', 'yen', 'local-nucleoli' and 'local-sg', 'local-sg1'
min_size_sg = 5
max_size_sg = 10000
nuclear_local_thresholding_size = 701
min_size_nuclear = 10000
max_size_nuclear = 130000
min_size_cell = 20000
max_size_cell = 1000000
low_stain_mode = 'off'  # only accepts 'on' and 'off'
watershed_mode = 'on'  # only accepts 'on' and 'off'

name = 'G3BP1_40X_200uMSA_1.25WGA'
imgs = nd2.ND2Reader('%s/%s.nd2' % (data_path, name))

# 0: SG channel
# 1: nuclei channel
# 2: cell boundary channel

# identify SG
sg = find_organelle(imgs[0], thresholding, 3000, 200, min_size_sg, max_size_sg)

# identify cell
cell, cell_ft = find_cell(imgs[0], imgs[2], imgs[1], nuclear_local_thresholding_size, min_size_nuclear,
                          max_size_nuclear, min_size_cell, max_size_cell)

with napari.gui_qt():
    # embed mpl widget in napari viewer
    viewer = napari.Viewer()
    viewer.add_image(np.array(imgs[2]), name='data_mem', colormap='red', blending='additive',
                     contrast_limits=(100, 1000))
    viewer.add_image(np.array(imgs[0]), name='data_sg', colormap='green', blending='additive')
    viewer.add_image(np.array(imgs[1]), name='data_nuclear', colormap='blue', blending='additive',
                     contrast_limits=(100, 7000))

    cmap1 = 'jet'
    cmap1_woBg = dis.num_color_colormap(cmap1, np.amax(cell))[0]
    viewer.add_image(cell, name='cell', colormap=('cmap1 woBg', cmap1_woBg), blending='additive')
